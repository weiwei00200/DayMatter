...
import com.pybeta.util.LunarCalendar;

/**
 * This class is a widget for selecting a date. The date can be selected by a
 * year, month, and day spinners or a {@link android.widget.CalendarView}. The
 * set of spinners and the calendar view are automatically synchronized. The
 * client can customize whether only the spinners, or only the calendar view, or
 * both to be displayed. Also the minimal and maximal date from which dates to
 * be selected can be customized.
 * <p>
...
public class DatePicker extends FrameLayout {
	...
	private LunarCalendar mCurrentLunar;
	
	private LunarCalendar mTempLunar;
	
	private String[] mShortLunarMonths;
	
	private String[] mShortLunarDays;

	private int mCurrentCalendar = CALENDAR_GREGORIAN;
	...
	public interface OnDateChangedListener {
		 ...
		 */
		void onDateChanged(DatePicker view, int calendar, int year, int monthOfYear, int dayOfMonth);
	}
	...
	public DatePicker(Context context, AttributeSet attrs, int defStyle) {
		...
		NumberPicker.OnValueChangeListener onChangeListener = new NumberPicker.OnValueChangeListener() {
			public void onValueChange(NumberPicker picker, int oldVal, int newVal) {
				...
				int tmpCalendar = mCurrentCalendar;
				mTempDate.setTimeInMillis(mCurrentDate.getTimeInMillis());
				mTempLunar.setDate(mCurrentLunar.getYear(), mCurrentLunar.getMonthOfYear(), mCurrentLunar.getDayOfMonth());
				
				// take care of wrapping of days and months to update greater fields
				if (picker == mDaySpinner) {
					if (mCurrentCalendar == CALENDAR_GREGORIAN) {
						...
						mTempLunar.setDayOfMonth(newVal);
					}
					...
					if (mCurrentCalendar == CALENDAR_GREGORIAN) {
						...
						mTempLunar.setMonthOfYear(newVal);
					}
				} else if (picker == mYearSpinner) {
					if (mCurrentCalendar == CALENDAR_GREGORIAN) {
						mTempDate.set(Calendar.YEAR, newVal);
					} else {
						mTempLunar.setYear(newVal);
					}
				} else if (picker == mCalendarSpinner) {
					tmpCalendar = newVal;
				} else {
					throw new IllegalArgumentException();
				}
				// now set the date to the adjusted one
				if (tmpCalendar == CALENDAR_GREGORIAN) {
					setDate(tmpCalendar, mTempDate.get(Calendar.YEAR), mTempDate.get(Calendar.MONTH), mTempDate.get(Calendar.DAY_OF_MONTH));
				} else {
					setDate(tmpCalendar, mTempLunar.getYear(), mTempLunar.getMonthOfYear(), mTempLunar.getDayOfMonth());
				}
				updateSpinners();
				notifyDateChanged();
			}
		...
		// calendar
		mCalendarSpinner = (NumberPicker) findViewById(R.id.calendar);
		mCalendarSpinner.setMinValue(0);
		mCalendarSpinner.setMaxValue(1);
		mCalendarSpinner.setDisplayedValues(mShortCalendars);
		mCalendarSpinner.setOnValueChangedListener(onChangeListener);
		...
		mCurrentCalendar = CALENDAR_GREGORIAN;
	 ...
	 * {@link com.pybeta.widget.DatePicker} in milliseconds since January 1,
	 * 1970 00:00:00 in {@link java.util.TimeZone#getDefault()} time zone.
	 * <p>
	 ...
	 * {@link android.widget.NumberPicker} in milliseconds since January 1, 1970
	 * 00:00:00 in {@link java.util.TimeZone#getDefault()} time zone.
	 * 
	 * @param minDate
	...
	private void setCurrentLocale(Locale locale) {
		...
		mShortLunarDays = getResources().getStringArray(R.array.date_picker_calendar_day);
		mShortLunarMonths = getResources().getStringArray(R.array.date_picker_calendar_month);
		mShortCalendars = getResources().getStringArray(R.array.date_picker_calendar);
		
		if (mCurrentLunar == null) {
			mCurrentLunar = new LunarCalendar();
		}
		if (mTempLunar == null) {
			mTempLunar = new LunarCalendar();
		}
	}
	...
	private void reorderSpinners() {
		...
		mSpinners.addView(mCalendarSpinner);
	...
	public void updateDate(int year, int month, int dayOfMonth) {
		setDate(CALENDAR_GREGORIAN, year, month, dayOfMonth);
	}
	 ...
	 *            The day of the month.
	 */
	public void updateDate(int calendar, int year, int month, int dayOfMonth) {
	...
	protected Parcelable onSaveInstanceState() {
		...
		return new SavedState(superState, getCalendar(), getYear(), getMonth(), getDayOfMonth());
	}
	...
	protected void onRestoreInstanceState(Parcelable state) {
		...
		setDate(ss.mCalendar, ss.mYear, ss.mMonth, ss.mDay);
		updateSpinners();
	}
	...
	public void init(int year, int monthOfYear, int dayOfMonth, OnDateChangedListener onDateChangedListener) {
		init(CALENDAR_GREGORIAN, year, monthOfYear, dayOfMonth, onDateChangedListener);
	}
	 ...
	 *            How user is notified date is changed by user, can be null.
	 */
	public void init(int calendar, int year, int monthOfYear, int dayOfMonth, OnDateChangedListener onDateChangedListener) {
	...
	private void setDate(int calendar, int year, int month, int dayOfMonth) {
		...
		if (mCurrentCalendar == CALENDAR_GREGORIAN) {
			...
			mCurrentLunar.setDate(year, month, dayOfMonth);
			if (year > DEFAULT_END_YEAR) {
				mCurrentLunar.setYear(DEFAULT_END_YEAR);
			} else if (year < DEFAULT_START_YEAR) {
				mCurrentLunar.setYear(DEFAULT_START_YEAR);
			}
		}
	}
	...
	private void updateLunarSpinners() {
		...
		mDaySpinner.setMaxValue(mCurrentLunar.getActualMaximum());
		mDaySpinner.setWrapSelectorWheel(true);
		
		// make sure the month names are a zero based array with the months in the month spinner
		String[] displayedDayValues = CVArrays.copyOfRange(mShortLunarDays, mDaySpinner.getMinValue(), mDaySpinner.getMaxValue() + 1);
		mDaySpinner.setDisplayedValues(displayedDayValues);
		...
		// make sure the month names are a zero based array with the months in the month spinner
		String[] displayedValues = CVArrays.copyOfRange(mShortLunarMonths, mMonthSpinner.getMinValue(), mMonthSpinner.getMaxValue() + 1);
		...
		mYearSpinner.setValue(mCurrentLunar.getYear());
		mMonthSpinner.setValue(mCurrentLunar.getMonthOfYear());
		mDaySpinner.setValue(mCurrentLunar.getDayOfMonth());
		mCalendarSpinner.setValue(mCurrentCalendar);
	}

	private void updateGregorianSpinners() {
		mDaySpinner.setDisplayedValues(null);
		// set the spinner ranges respecting the min and max dates
		...
		// make sure the month names are a zero based array with the months in the month spinner
		String[] displayedValues = CVArrays.copyOfRange(mShortMonths, mMonthSpinner.getMinValue(), mMonthSpinner.getMaxValue() + 1);
		mMonthSpinner.setDisplayedValues(displayedValues);
	...
	public int getMonth() {
		if (mCurrentCalendar == CALENDAR_GREGORIAN) {
			...
			return mCurrentLunar.getMonthOfYear();
		}
	}
	...
	public int getDayOfMonth() {
		if (mCurrentCalendar == CALENDAR_GREGORIAN) {
			...
			return mCurrentLunar.getDayOfMonth();
		}
	}
	...
	private void notifyDateChanged() {
		...
		if (mOnDateChangedListener != null) {
			mOnDateChangedListener.onDateChanged(this, mCurrentCalendar, getYear(), getMonth(), getDayOfMonth());
		}
	}
	...
	private static class SavedState extends BaseSavedState {
		 ...
		 * {@link com.pybeta.widget.DatePicker#onSaveInstanceState()}
		 */
		private SavedState(Parcelable superState, int calendar, int year, int month, int day) {
			super(superState);
			mCalendar = calendar;
			mYear = year;
			mMonth = month;
