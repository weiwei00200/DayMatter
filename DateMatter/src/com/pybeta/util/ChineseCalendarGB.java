...
public class ChineseCalendarGB {
	...
	public static int dayOfChineseYear(int y, int m, int d) {
		int days = d;

		int index = 1;
		while(index < m) {
			days += daysInChineseMonth(y, index);
			index = nextChineseMonth(y, index);
		}
		
		return days;
	}
	
	public static int daysInChineseYear(int y) {
		int days = 0;
	...
	// 初始日，公曆農曆對應日期：
	// 公曆 1901 年 1 月 1 日，對應農曆 1900 年 11 月 11 日
	private static int baseYear = 1901;
	private static int baseMonth = 1;
	private static int baseDate = 1;
	private static int baseIndex = 0;
	private static int baseChineseYear = 1900;
	...
	public int computeChineseFields() {
		...
		// 第二個對應日，用以提高計算效率
		// 公曆 2000 年 1 月 1 日，對應農曆 1999 年 11 月 25 日
	...
	// 计算两个农历日期之间的天数
	public static int computeDistance(int y1, int m1, int d1, int y2, int m2, int d2) {
		int days = 0;
		...
		if (y1 == y2) {
			days = daysInChineseMonth(y1, m1) - d1; 
			for (int index = m1 + 1; index < m2; index++) {
				days += daysInChineseMonth(y1, index);
			}
			days += d2;
			return days;
		}

		days = daysInChineseYear(y1) - dayOfChineseYear(y1, m1, d1);
		for (int index = y1 + 1; index < y2; index++) {
			days += daysInChineseYear(index);
		}
		days += dayOfChineseYear(y2, m2, d2);
		return days;
	}
